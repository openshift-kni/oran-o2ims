...
func findNextNodeToUpdate(nodelist *pluginsv1alpha1.AllocatedNodeList, groupname, newHwProfile string) *pluginsv1alpha1.AllocatedNode {
	for _, node := range nodelist.Items {
		...
		// Profile is already set — but check if it failed and needs retry
		cond := meta.FindStatusCondition(node.Status.Conditions, string(hwmgmtv1alpha1.Configured))
		if cond == nil || cond.Reason == string(hwmgmtv1alpha1.InvalidInput) || cond.Reason == string(hwmgmtv1alpha1.Failed) {
			// retry this node
			return &node
		}
	}
...
func processNodeAllocationRequestAllocation(
	...
	for _, nodeGroup := range nodeAllocationRequest.Spec.NodeGroup {
		...
		unallocatedBMHs, err := fetchBMHList(
			ctx, c, logger,
			nodeAllocationRequest.Spec.Site,
			nodeGroup.NodeGroupData, UnallocatedBMHs,
		)
		if err != nil {
			// treat as transient read issue → short retry
			return hwmgrutils.RequeueWithShortInterval(),
				fmt.Errorf("fetch unallocated BMHs for site=%s, nodegroup=%s: %w",
