...
func checkNodeAllocationRequestProgress(
	...
	if !full {
		return false, processNodeAllocationRequestAllocation(ctx, c, noncachedClient, logger, pluginNamespace, nodeAllocationRequest)
	}
...
func handleInProgressUpdate(ctx context.Context,
	...
	if bmh.Status.OperationalStatus == metal3v1alpha1.OperationalStatusOK {
		...
		if err := clearTransientBMHErrorAnnotation(ctx, c, logger, bmh); err != nil {
			...
			return hwpluginutils.RequeueImmediately(), true, err
		}
...
func allocateBMHToNodeAllocationRequest(ctx context.Context,
	...
	if !updating {
		if err := clearBMHNetworkData(ctx, c, types.NamespacedName{Name: bmh.Name, Namespace: bmh.Namespace}); err != nil {
