...
var _ = Describe("Helpers", func() {
	...
	Describe("Node Finding Functions", func() {
		...
		Describe("findNextNodeToUpdate", func() {
			...
			It("should return node with failed condition for retry even if profile matches", func() {
				// Add node with matching profile but failed condition
				failedNode := pluginsv1alpha1.AllocatedNode{
					ObjectMeta: metav1.ObjectMeta{Name: "failed-node"},
					Spec: pluginsv1alpha1.AllocatedNodeSpec{
						GroupName: "test-group",
						HwProfile: "target-profile", // Same profile as target
					},
					Status: pluginsv1alpha1.AllocatedNodeStatus{
						Conditions: []metav1.Condition{
							{
								Type:   string(hwmgmtv1alpha1.Configured),
								Status: metav1.ConditionFalse,
								Reason: string(hwmgmtv1alpha1.Failed), // Failed nodes should be retried
							},
						},
					},
				}
				nodeList.Items = append(nodeList.Items, failedNode)

				result := findNextNodeToUpdate(nodeList, "test-group", "target-profile")
				Expect(result).NotTo(BeNil())
				Expect(result.Name).To(Equal("failed-node"))
			})
	...
	Describe("Complex Integration Functions - Skipped", func() {
		...
		Context("handleInProgressUpdate", func() {
			...
			BeforeEach(func() {
				...
				// Create a test BMH in error state
				testBMH = &metal3v1alpha1.BareMetalHost{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "test-bmh",
						Namespace: "test-namespace",
			...
			It("should clean up config annotation when BMH is in error state", func() {
				...
				Expect(handled).To(BeFalse()) // Should return false when error occurs
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("failed to apply changes"))
