...
import (
	...
	ctrlutils "github.com/openshift-kni/oran-o2ims/internal/controllers/utils"
...
func (s *NodeAllocationRequestCallbackServer) Serve(ctx context.Context, config utils.CommonServerConfig) error {
	...
	serverTLSConfig, err := ctrlutils.GetServerTLSConfig(ctx, config.TLS.CertFile, config.TLS.KeyFile)
	if err != nil {
		return fmt.Errorf("failed to get NAR Callback server TLS config: %w", err)
	}

	port, err := ctrlutils.ExtractPortFromAddress(config.Listener.Address)
	if err != nil {
...
func (n *NodeAllocationRequestCallbackServer) ProvisioningRequestCallback(ctx context.Context, request ProvisioningRequestCallbackRequestObject) (ProvisioningRequestCallbackResponseObject, error) {
	...
	pr.Annotations[ctrlutils.CallbackReceivedAnnotation] = fmt.Sprintf("%d", time.Now().Unix())
	pr.Annotations[ctrlutils.CallbackStatusAnnotation] = string(request.Body.Status)
	pr.Annotations[ctrlutils.CallbackNodeAllocationRequestIdAnnotation] = request.Body.NodeAllocationRequestId
	...
	n.Logger.Info("Successfully processed callback",
		"provisioningRequestName", request.ProvisioningRequestName,
		"nodeAllocationRequestId", request.Body.NodeAllocationRequestId,
