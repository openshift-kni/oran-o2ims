...
func setServerFlags(cmd *cobra.Command) {
	...
	flags.StringVar(
		&config.Listener.CertFile,
		server.APIListenerTLSCertFlagName,
		"/secrets/tls/tls.crt",
		"API TLS certificate in PEM format.",
	)
	flags.StringVar(
		&config.Listener.KeyFile,
		server.APIListenerTLSKeyFlagName,
		"/secrets/tls/tls.key",
		"API TLS key in PEM format.",
	)
	flags.StringVar(
		&config.CloudID,
