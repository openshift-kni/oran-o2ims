...
func (c *Collector) syncThanosAlarmDefinitions(ctx context.Context, ds *AlarmsDataSource) error {
	...
	if err != nil {
		return fmt.Errorf("failed to upsert Thanos alarm definitions: %w", err)
	}
...
func (c *Collector) syncAlarmDictionaries(ctx context.Context, ds *AlarmsDataSource, nodeClusterTypes []models.NodeClusterType) error {
	...
	// Get thanos alarm definitions
	thanosAlarmDefinitions, err := c.repository.GetThanosAlarmDefinitions(ctx)
	if err != nil {
		return fmt.Errorf("failed to get thanos alarm definitions: %w", err)
	}
	...
	for _, value := range alarmDictionaries {
		...
		g.Go(func() error {
			...
			// Append thanos alarm definitions to the alarm dictionary
			alarmDefinitions = append(alarmDefinitions, thanosAlarmDefinitions...)
