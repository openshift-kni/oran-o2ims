...
CREATE TABLE IF NOT EXISTS alarm_event_record (
    ...
    global_cloud_id UUID, -- Global cloud ID taken from the server
...
AFTER INSERT OR UPDATE is called to insert to outbox if should_create_data_change_event is true or a brand new row.
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        ELSIF NOT NEW.alarm_acknowledged THEN
            IF (NEW.object_id IS DISTINCT FROM OLD.object_id OR
                ...
                NEW.global_cloud_id IS DISTINCT FROM OLD.global_cloud_id OR
...
-- AFTER trigger function: Insert into outbox if should_create_data_change_event or insert
CREATE OR REPLACE FUNCTION manage_alarm_event_after()
RETURNS TRIGGER AS $$
BEGIN
   IF (TG_OP = 'INSERT') OR (TG_OP = 'UPDATE' AND NEW.should_create_data_change_event = TRUE) THEN
