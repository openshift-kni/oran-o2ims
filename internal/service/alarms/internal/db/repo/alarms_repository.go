...
func (ar *AlarmsRepository) UpdateSubscriptionEventCursor(ctx context.Context, subscription models.AlarmSubscription, latestSequence int64) error {
	m := models.AlarmSubscription{}
	dbTags := utils.GetAllDBTagsFromStruct(m)
	query := psql.Update(
		um.Table(m.TableName()),
		um.SetCol(dbTags["EventCursor"]).ToArg(latestSequence),
		um.Where(psql.Quote(dbTags["SubscriptionID"]).EQ(psql.Arg(subscription.SubscriptionID))),
		um.Returning(dbTags["EventCursor"]),
	)

	sql, params, err := query.Build()
	if err != nil {
		return fmt.Errorf("failed to build UpdateSubscriptionEventCursor query: %w", err)
	}

	record, err := utils.ExecuteCollectExactlyOneRow[models.AlarmSubscription](ctx, ar.Db, sql, params)
	if err != nil {
		return fmt.Errorf("failed to execute UpdateSubscriptionEventCursor query: %w", err)
	}

	slog.Info("Successfully updated subscription with new event cursor", "subscription", subscription.SubscriptionID, "from", subscription.EventCursor, "to", record.EventCursor)
	return nil
}
