...
// ClusterCollector collects alarms from a cluster
...
// loadClusterDictionaries loads the alarm dictionaries for a cluster
func (r *ClusterCollector) loadClusterDictionaries(ctx context.Context, infraClient infrastructure.Client) {
	slog.Info("loading alarm dictionaries", "server", infraClient.Name())

	// Copy the node cluster types to avoid locking the client for too long
	nodeClusterTypes := make([]clusterserver.NodeClusterType, len(*infraClient.(*clusterserver.ClusterServer).NodeClusterTypes))
	infraClient.(*clusterserver.ClusterServer).Lock()
	if infraClient.(*clusterserver.ClusterServer).NodeClusterTypes == nil {
		slog.Warn("no node cluster types to load")
		infraClient.(*clusterserver.ClusterServer).Unlock()
		return
	}

	slog.Info(fmt.Sprintf("Copying %v node cluster types", *infraClient.(*clusterserver.ClusterServer).NodeClusterTypes))
	copy(nodeClusterTypes, *infraClient.(*clusterserver.ClusterServer).NodeClusterTypes)
	infraClient.(*clusterserver.ClusterServer).Unlock()
...
func (r *ClusterCollector) processHub(ctx context.Context) ([]monitoringv1.Rule, error) {
...
func (r *ClusterCollector) processManagedCluster(ctx context.Context, version string) ([]monitoringv1.Rule, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error creating client for hub: %w", err)
	}

	cl, err := getClientForCluster(ctx, hubClient, cluster.Name)
	if err != nil {
		return nil, err
	}
...
func (r *ClusterCollector) getManagedCluster(ctx context.Context, version string) (*clusterv1.ManagedCluster, error) {
...
func (r *ClusterCollector) getRules(ctx context.Context, cl crclient.Client) ([]monitoringv1.Rule, error) {
	ctxWithTimeout, cancel := context.WithTimeout(ctx, clients.ListRequestTimeout)
	defer cancel()
...
func (r *ClusterCollector) syncDictionaries(ctx context.Context, nodeClusterTypes []clusterserver.NodeClusterType) {
