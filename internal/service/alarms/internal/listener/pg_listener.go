...
func ListenForAlarmsPgChannels(ctx context.Context, pool *pgxpool.Pool, a *api.AlarmsServer) error {
	...
	lm.RegisterListener("alarm_event_record_outbox_queued",
		// Function called after a notification is received,
		func(ctx context.Context, pgNotification *pgconn.Notification) error {
			return processOutboxNotification(ctx, a, "primary")
		},
		// Catch-up function
		func(ctx context.Context) error {
			return processOutboxNotification(ctx, a, "backup")
		},
		// Catch-up interval
		15*time.Minute,
	)
...
func processOutboxNotification(ctx context.Context, a *api.AlarmsServer, caller string) error {
	dataChangeEvents, err := a.AlarmsRepository.ClaimDataChangeEvent(ctx)
	if err != nil {
		return fmt.Errorf("failed to claim notifications: %w", err)
	}

	if len(dataChangeEvents) > 0 && caller == "backup" {
		slog.Warn("outbox is expected to empty but found at least one data change event")
	}
	...
	for _, dataChangeEvent := range dataChangeEvents {
		notification, err := models.DataChangeEventToNotification(&dataChangeEvent)
		if err != nil {
