...
var _ = Describe("ProvisioningServer", func() {
	...
	Describe("GetProvisioningRequest strips BMC credentials", func() {
		var (
			server *api.ProvisioningServer
			scheme *runtime.Scheme
		)
		...
		It("should not include bmcCredentialsDetails in the response", func() {
			templateParams := map[string]interface{}{
				"clusterInstanceParameters": map[string]interface{}{
					"clusterName": "test-cluster",
					"nodes": []interface{}{
						map[string]interface{}{
							"hostName":   "node1",
							"bmcAddress": "idrac-virtualmedia+https://203.0.113.5/redfish/v1/Systems/System.Embedded.1",
							"bmcCredentialsDetails": map[string]interface{}{
								"username": "admin",
								"password": "s3cret",
							},
						},
			...
			result := resp.(provisioningapi.GetProvisioningRequest200JSONResponse)
			ciParams, ok := result.ProvisioningRequestData.TemplateParameters["clusterInstanceParameters"].(map[string]interface{})
			Expect(ok).To(BeTrue())
			...
			Expect(ok).To(BeTrue())
			Expect(nodeMap).ToNot(HaveKey("bmcCredentialsDetails"))
			Expect(nodeMap).To(HaveKey("hostName"))
			Expect(nodeMap).To(HaveKey("bmcAddress"))
		})
	})
