...
`
	...
	It("allows a defaults group missing from the ProvisioningRequest", func() {
		defaults := `
nodeGroups:
- name: master
  role: master
...
`
		...
		Expect(nodes[0].(map[string]any)["role"]).To(Equal("master"))
	})

	It("allows an empty nodes list for a defaults group present in the ProvisioningRequest", func() {
		defaults := `
nodeGroups:
- name: master
  role: master
