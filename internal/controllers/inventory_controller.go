...
// waitForDeployments waits for all server pods to be ready.  If any pod is not yet ready, the reconciler timer is set
// to a short value so that we can try again quickly; otherwise either an error is returned without a reconciler timer
// to indicate that an exponential backoff is required
func (t *reconcilerTask) waitForPods(ctx context.Context) (nextReconcile ctrl.Result, err error) {
...
func (t *reconcilerTask) run(ctx context.Context) (nextReconcile ctrl.Result, err error) {
	...
	nextReconcile, err = t.waitForPods(ctx)
	if err != nil {
		return
	}

	if !nextReconcile.IsZero() {
		// The pods are ready and we are going to retry
		return
	}
