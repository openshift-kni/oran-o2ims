...
func (t *provisioningRequestReconcilerTask) updateClusterInstance(ctx context.Context,
	...
	hwNodes, err := utils.CollectNodeDetails(ctx, nodes)
	if err != nil {
	...
	if hwpluginRef != utils.Metal3PluginName {
...
func (t *provisioningRequestReconcilerTask) applyNodeConfiguration(
	...
	for i, n := range nodes {
		...
		// Update node status : TODO -- remove / refactor when onboarding the metal3 HardwarePlugin
