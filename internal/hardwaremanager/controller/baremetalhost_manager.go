...
func processHwProfileWithHandledError(
	...
	updateRequired, err := processHwProfile(ctx, c, logger, namespace, bmh, profileName, postInstall, validateOnly)
...
func processHwProfile(ctx context.Context,
	...
	// Resolve firmware catalog references to internal types
	resolved, err := resolveFirmwareFromCatalog(ctx, c, namespace, hwProfile.Spec)
	if err != nil {
		return false, fmt.Errorf("failed to resolve firmware from catalog for profile %s: %w", profileName, err)
	}

	// Check if firmware update is required
	logger.DebugContext(ctx, "Checking if firmware update is required")
	firmwareUpdateRequired, err := IsFirmwareUpdateRequired(ctx, c, logger, bmh, resolved, validateOnly)
	if err != nil {
		return false, err
	}
...
func handleTransitionNodes(ctx context.Context,
	...
	for _, node := range nodelist.Items {
		res, err := handleTransitionNode(ctx, c, noncachedClient, logger, namespace, &node, postInstall, nil)
...
func handleTransitionNode(ctx context.Context,
	...
	for _, uc := range updateCases {
		...
		res, err := processBMHUpdateCase(ctx, c, noncachedClient, logger, namespace, node, bmh, uc, postInstall, nodeOps)
		if err != nil || res.Requeue || res.RequeueAfter > 0 {
...
func removeInfraEnvLabelFromPreprovisioningImage(ctx context.Context, c client.Client, logger *slog.Logger,
...
// deleteIBINetworkDataSecret deletes the IBI-generated network-data secret
// (named after the BMH) and clears the PPI's networkDataName. IBI provisioning
// creates this secret with ownerReferences to both the BMH and PPI, and BMO
// protects it with a finalizer. Deleting it during IBI post-provisioning
...
func deleteIBINetworkDataSecret(ctx context.Context, c client.Client, logger *slog.Logger, bmh *metal3v1alpha1.BareMetalHost) error {
	...
	if err := c.Get(ctx, secretName, secret); err != nil {
		...
		// Validate the secret is IBI-generated by checking for BMH and PPI
		// ownerReferences. If either is missing, this isn't an IBI secret
		// and we should not delete it.
		if !isIBIGeneratedSecret(secret, bmh.Name) {
			logger.InfoContext(ctx, "Secret does not have expected IBI ownerReferences, skipping deletion",
		...
		logger.InfoContext(ctx, "Deleted IBI network-data secret",
			slog.String("secret", secretName.String()))
	}
	...
	if err := retry.OnError(retry.DefaultRetry, k8serrors.IsConflict, func() error {
		var current metal3v1alpha1.BareMetalHost
		if err := c.Get(ctx, bmhName, &current); err != nil {
			return fmt.Errorf("failed to get BMH %s: %w", bmhName, err)
		}
		if current.Spec.PreprovisioningNetworkDataName == origValue {
			return nil
		}
		patch := client.MergeFrom(current.DeepCopy())
		current.Spec.PreprovisioningNetworkDataName = origValue
...
// isIBIGeneratedSecret checks whether a secret was created by IBI provisioning
// by verifying it has ownerReferences to both the BareMetalHost and
// PreprovisioningImage with the given name.
func isIBIGeneratedSecret(secret *corev1.Secret, bmhName string) bool {
	hasBMHOwner := false
	hasPPIOwner := false
	for _, ref := range secret.OwnerReferences {
		if ref.Kind == "BareMetalHost" && ref.Name == bmhName {
			hasBMHOwner = true
		}
		if ref.Kind == "PreprovisioningImage" && ref.Name == bmhName {
			hasPPIOwner = true
		}
	}
	return hasBMHOwner && hasPPIOwner
}
