...
func clearConfigAnnotationWithPatch(ctx context.Context, c client.Client, node *hwmgmtv1alpha1.AllocatedNode) error {
...
func hasUpdateAbandonedAnnotation(node *hwmgmtv1alpha1.AllocatedNode) bool {
...
func setUpdateAbandonedAnnotation(ctx context.Context, c client.Client, node *hwmgmtv1alpha1.AllocatedNode) error {
	patch := client.MergeFrom(node.DeepCopy())
	if node.Annotations == nil {
...
func clearUpdateAbandonedAnnotation(ctx context.Context, c client.Client, node *hwmgmtv1alpha1.AllocatedNode) error {
...
func findNodesInProgress(nodelist *hwmgmtv1alpha1.AllocatedNodeList) []*hwmgmtv1alpha1.AllocatedNode {
...
func applyPostConfigUpdates(ctx context.Context,
	c client.Client,
	noncachedClient client.Reader,
	node *hwmgmtv1alpha1.AllocatedNode) (int, error) {
...
func getGroupsSortedByRole(nodeAllocationRequest *hwmgmtv1alpha1.NodeAllocationRequest) []hwmgmtv1alpha1.NodeGroup {
	...
	groups := append([]hwmgmtv1alpha1.NodeGroup(nil), nodeAllocationRequest.Spec.NodeGroup...)
...
func checkNodeAllocationRequestProgress(
	...
	full, err := isNodeAllocationRequestFullyAllocated(ctx, noncachedClient, logger, namespace, nodeAllocationRequest)
	if err != nil {
		return ctrl.Result{}, false, err
	}
	if !full {
		// Still not fully allocated, continue processing
		res, err := processNodeAllocationRequestAllocation(ctx, c, noncachedClient, logger, namespace, nodeAllocationRequest)
		return res, false, err
	}

	// check if there are any pending work such as bios configuring
	res, updating, err := checkForPendingUpdate(ctx, c, noncachedClient, logger, namespace, nodeAllocationRequest)
...
func initiateNodeUpdate(ctx context.Context,
	...
	validateOnly := true
	updateNeeded, err := processHwProfileWithHandledError(ctx, c, noncachedClient, logger, namespace,
...
func selectNodesToProcess(
	...
	for _, nodegroup := range getGroupsSortedByRole(nar) {
		...
		var candidates = []*hwmgmtv1alpha1.AllocatedNode{}
...
type nodeClassification struct {
	...
	FailedNodes []*hwmgmtv1alpha1.AllocatedNode
	...
	PriorityNodes []*hwmgmtv1alpha1.AllocatedNode
	// PendingNodes: ready nodes without special conditions that need to be evaluated/updated
	PendingNodes []*hwmgmtv1alpha1.AllocatedNode
}

func classifyNodes(
	ctx context.Context, logger *slog.Logger, nodeOps NodeOps,
	nodes []*hwmgmtv1alpha1.AllocatedNode, newHwProfile string,
) nodeClassification {
	var nc nodeClassification
	var notReadyNodes, abandonedNodes []*hwmgmtv1alpha1.AllocatedNode
...
func getNewHwProfileForNode(nar *hwmgmtv1alpha1.NodeAllocationRequest, node *hwmgmtv1alpha1.AllocatedNode) string {
...
func filterNodesByGroup(nodelist *hwmgmtv1alpha1.AllocatedNodeList, groupName string) []*hwmgmtv1alpha1.AllocatedNode {
	var result []*hwmgmtv1alpha1.AllocatedNode
	for i := range nodelist.Items {
...
func allocateBMHToNodeAllocationRequest(
	...
	node, err := createNode(ctx, c, logger, namespace, nodeAllocationRequest, nodeName, nodeId, nodeNs, group.NodeGroupData.Name, group.NodeGroupData.HwProfile)
	if err != nil {
	...
	if err := updateNodeStatus(ctx, c, noncachedClient, logger, namespace, nodeInfo, nodeName, group.NodeGroupData.HwProfile, updating); err != nil {
...
func isNodeProvisioningInProgress(allocatednode *hwmgmtv1alpha1.AllocatedNode) bool {
...
func validateFirmwareVersions(
	...
	// 2) Resolve firmware from catalog
	fw, err := resolveFirmwareFromCatalog(ctx, c, namespace, prof.Spec)
	if err != nil {
		return false, fmt.Errorf("resolve firmware for HardwareProfile %s: %w", hwProfileName, err)
	}

	// Build expected versions map (normalized)
	expected := map[string]string{}
	if v := strings.TrimSpace(fw.BiosFirmware.Version); v != "" {
		expected["bios"] = normalizeVersion(v)
	}
	if v := strings.TrimSpace(fw.BmcFirmware.Version); v != "" {
...
func validateAppliedBiosSettings(
	...
	// 5) Resolve firmware from catalog for NIC validation
	fwResolved, err := resolveFirmwareFromCatalog(ctx, c, namespace, prof.Spec)
	if err != nil {
		return false, fmt.Errorf("resolve firmware for NIC validation in HardwareProfile %s: %w", hwProfileName, err)
	}

	// Validate NIC firmware if specified
	if len(fwResolved.NicFirmware) > 0 {
		...
		for i, nic := range fwResolved.NicFirmware {
			if nic.Version == "" {
