...
If `firmwareImages` is non-empty, the deprecated `biosFirmware`, `bmcFirmware`, and
...
4. **Firmware diff computation** — The hardware manager compares the HardwareProfile's desired
   ...
   - **Firmware versions:** Resolves the desired firmware versions (from `firmwareImages`
     via the FirmwareCatalog, or from the deprecated inline
     `biosFirmware`/`bmcFirmware`/`nicFirmware` fields) and compares them against the
     host's `HostFirmwareComponents` status. If any version differs, firmware update is
     needed.
