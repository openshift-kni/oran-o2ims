...
worker nodes by updating hosts under the worker group’s
`clusterInstanceParameters.nodeGroups[].nodes` list in the
ProvisioningRequest.
...
* The worker group must exist in the ClusterInstance defaults for the
...
1. Ensure the ClusterTemplate / ClusterInstance defaults include the
   ...
   > ClusterInstance defaults and hardware `nodeGroupData` include the
