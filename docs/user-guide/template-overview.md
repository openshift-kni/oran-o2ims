...
- [Template Overview](#template-overview)
  - [ClusterTemplate CR](#clustertemplate-cr)
    ...
    - [Template schema](#template-schema)
      ...
      - [upgradeParameters schema](#upgradeparameters-schema)
...
- templateDefaults: Contains default values for templates.
  ...
  - upgradeDefaults: Optional inline defaults for cluster upgrade. See [Cluster Upgrade](./upgrade.md).
...
See the [hwMgmtParameters sample](../samples/schema/hwMgmtParameters.yaml) for the schema
...
| --- | --- | --- | --- |
| `nodeGroups` | Recommended | O-Cloud extension: hosts are grouped; node-group-level configuration remains ClusterInstance-shaped | [`clusterInstanceParameters-nodeGroups.yaml`](../samples/schema/clusterInstanceParameters-nodeGroups.yaml) |
| `nodes` | Deprecated | Flat top-level ClusterInstance `nodes` subschema. Still accepted for existing templates; new templates must use `nodeGroups` | [`clusterInstanceParameters-legacy-nodes.yaml`](../samples/schema/clusterInstanceParameters-legacy-nodes.yaml) |
...
allows the ProvisioningRequest to override settings from the ClusterTemplate's
`upgradeDefaults`. The schema must define **exactly one** upgrade type:

| Upgrade type | Cluster | Sample schema |
| --- | --- | --- |
| `imageBasedGroupUpgrade` | SNO IBU | [`upgradeParameters-imageBasedGroupUpgrade.yaml`](../samples/schema/upgradeParameters-imageBasedGroupUpgrade.yaml) |
| `clusterVersion` | MNO ClusterVersion | [`upgradeParameters-clusterVersion.yaml`](../samples/schema/upgradeParameters-clusterVersion.yaml) |

Defining both `imageBasedGroupUpgrade` and `clusterVersion` in the same schema
is rejected. When `upgradeDefaults` is set, `upgradeParameters` is required
and must match the upgrade type used in the defaults.

See [Cluster Upgrade](./upgrade.md) for the full upgrade workflows.
...
See the [policyTemplateParameters sample](../samples/schema/policyTemplateSchema.yaml)
for the schema definition of `policyTemplateParameters`.
