...
test tests:
	...
	time timeout --kill-after=30s 60m ginkgo run -vv -r \
		--compilers=1 --label-filter="!envtest" --no-color --poll-progress-after=30s --poll-progress-interval=30s --timeout=60m \
